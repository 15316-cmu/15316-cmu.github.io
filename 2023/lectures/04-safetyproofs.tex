...
The \dfn[valuation!of~programs]{trace semantics $\iac[\alpha]{\I}$ of a program}~$\alpha$ is the set of all its possible traces and is defined inductively as follows:
    % \index{_\tau(\alpha)_@$\tau(\alpha)$}%
    \begin{enumerate}
    \item
      \m{\iac[\pupdate{\umod{x}{\astrm}}]{\I}}
      =
   ...
   \item \(\iac[\passert{\ivr}]{\I} = \{(\ws,\ws) \with
      \imodels{\I}{\ivr}\} \cup
        \{(\ws,\errstate) \with
      \inonmodels{\I}{\ivr}\}\)
      \index{$\ptest{}$} \\
    ...
    \item \(\iac[\pif{\ivr}{\alpha}{\beta}]{\I} =
      \{\atrace \in \iac[\alpha]{\I} \with \atrace_0 \models \ivr\} \cup
      \{\atrace \in \iac[\beta]{\I} \with \atrace_0 \nonmodels \ivr\}\) \\
      The \m{\pif{\ivr}{\asprg}{\bsprg}} program runs $\asprg$ if $\ivr$ is true in the initial state and otherwise runs $\bsprg$.
    
    \item \(\iac[{\alpha};{\beta}]{\I} =
      \{\atrace \compose \varsigma \with \atrace\in\iac[\alpha]{\I} \mand \varsigma\in\iac[\beta]{\I}\}\);\\
      ...
      \end{cases}
      \] \\
      The relation \m{\iac[\asprg;\bsprg]{\I}} is the composition of traces from \(\iac[\bsprg]{\I}\) after those from \(\iac[\asprg]{\I}\) and can, thus, follow any transition of $\asprg$ through any intermediate state $\iget[state]{\Iz}$ to a transition of $\bsprg$.
...
\begin{lemma}[Determinism] \label{lem:determinism}
  The programs $\asprg$ from \rref{def:deterministic-program} are \dfn{deterministic}, that is, for every initial state $\iget[state]{\I}$ there is at most one trace $\atrace$ such that $\sigma \in \iac[\asprg]{\I}$ and $\sigma_0 = \iget[state]{\I}$.
\end{lemma}
\begin{proof}
...
The next case to look into is what we need to prove in order to show the formula \m{\dbox{\pupdate{\umod{x}{\astrm}}}{p(x)}}, which expresses that the formula $p(x)$ holds after the assignment \m{\pupdate{\umod{x}{\astrm}}} that assigns the value of term $\astrm$ to variable $x$. The first instinct might be the following axiom for assignment:
\[
...
\linfer[implyr+andl] {
  ...
  \lsequent{}{y \le 1 \land x \ne 0 \limply \dbox{z:=0; \pwhile{y>0}{\asprg}}{z \ne 0}}
}
\end{sequentdeduction}
Now we can proceed no further without applying \irref{unfold}.
...
\end{sequentdeduction}
Now in both branches of the proof, we can't go any further without applying \irref{unfold}.
However, notice that the assumptions of both sequents now have among them:
\[
y \le 1~\text{and}~y_1 = y/2
\]
We know that the integer part of $y_1$ will be 0, so when we unfold again, and move the condition $y_1 > 0$ into our assumptions, this will create an immediate contradiction, and both branches will be valid by arithmetic: we can close out with \irref{qear}.
...
\end{align*}
But note that these are purely formulas of arithmetic, and if they are valid, then we can close them with \irref{qear} as well.
This completes our analysis of the program with \irref{unfold}.
...
\end{align*}
Although we closed out the remaining two sequents with \irref{qear}, they did not correspond to formulas entirely of arithmetic, as they still each contained a box modality with the remainder of the loop to execute.

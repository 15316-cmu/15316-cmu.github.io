...
\newcommand{\lecnum}{5}
\newcommand{\lectitle}{Bounded Model Checking and Symbolic Execution}
...
First, we have used a new rule that hasn't been mentioned before, \irref{assignbeqr}.
\begin{center}
\begin{calculus}
...
\linfer[assignbeqr] {
  \lsequent{x=0,x=1}{x=0}
} {
  \lsequent{x = 0}{\dbox{x:=1}{x=0}}
}
\end{sequentdeduction}
We end up with a contradiction $x=0\land x=1$ in our assumptions, from which we can prove anything. This is obviously not sound, because we should not be able to prove that $x=0$ immediately after assigning $x:=1$! The correct application of \irref{assignbeqr} in this case would be as follows.
\begin{sequentdeduction}
\linfer[assignbeqr] {
  \lsequent{x=0,y=1}{y=0}
} {
  \lsequent{x = 0}{\dbox{x:=1}{x=0}}
}
\end{sequentdeduction}
...
The second thing to notice is that we can proceed no further in the proof without applying \irref{unfold}. So we proceed to unfold the loop, and apply non-loop axioms afterwards until we get stuck again.
\begin{sequentdeduction}
\linfer[unfold] {
  \linfer[andr] {
    \linfer[implyr] {

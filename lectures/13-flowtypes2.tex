...
\label{thm:soundness}
Let $\asprg$ be a program, and $\omega_1, \omega_2$ be states such that $\omega_1 \approx_{\Gamma,\lowsec} \omega_2$. 
Moreover, let $\omega_1', \omega_2'$ be states such that $\langle\omega_1,\asprg\rangle\bigstep\omega_1'$ and $\langle\omega_2,\asprg\rangle\bigstep\omega_2'$. 
Then for a policy $\Gamma$ over the lattice $\{\lowsec, \hisec\}$, if $\lsequent{\Gamma}{\asprg}$ then $\omega_1' \approx_{\Gamma,\lowsec} \omega_2'$.
\end{theorem}
...
\text{ and }
\langle \omega_2, \pwhile{\ivr}{\asprg}\rangle \Downarrow \omega_2'
\]
it must be the case that $\omega_1' \approx_{\Gamma,\lowsec} \omega_2'$.
Using the fact that $\lsequent{\Gamma}{\pwhile{\ivr}{\asprg}}$, we proceed in cases on the label $\ell'$ used to type the subcommand $\alpha$ by the (\irref{whileflow}) rule, reproduced below for convenience.
\[
\cinferenceRule[whileflow|While]{while}
{\linferenceRule[sequent] {
  \lsequent{\Gamma}{\ivr : \ell}
...
\]
Either the label $\ell'$ used to type the subcommand $\asprg$ is \lowsec or it is \hisec.
\begin{description}
\item[Case $\ell' = \lowsec$:] Simple security (Lemma~\ref{lemma:simplesec}) tells us that for any variable $x$ in $\astrm$, $\Gamma(x) \posetleq \lowsec$. 
But $\lowsec$ is the global bottom element of the lattice $\{\lowsec, \hisec\}$, and because $\posetleq$ is transitive, $\Gamma(x) = \lowsec$. 
We can then use the fact that $\omega_1 \approx_{\Gamma,\lowsec} \omega_2$ to reason that for some constant $b$, $\langle\omega_1,\ivr\rangle\bigstepb b$ and $\langle\omega_2,\ivr\rangle\bigstepb b$. In other words, $\ivr$ evaluates to the same thing in $\omega_1$ and $\omega_2$, so both executions take the same branch through the loop condition.
...
If $b = \ltrue$, then things are a bit more nuanced. We know that the last steps of the big-step derivations of were:
\[
\linferenceRule{
  \langle\omega_1,\ivr\rangle \bigstep_{\mathbb{B}} \ltrue
  &\langle\omega_1,\asprg;\pwhile{\ivr}{\asprg}\rangle \bigstep \omega_1'
}{
  \langle\omega_1,\pwhile{\ivr}{\asprg}\rangle \bigstep \omega_1'
}
\quad
\linferenceRule{
  \langle\omega_2,\ivr\rangle \bigstep_{\mathbb{B}} \ltrue
  &\langle\omega_2,\asprg;\pwhile{\ivr}{\asprg}\rangle \bigstep \omega_2'
}{
  \langle\omega_2,\pwhile{\ivr}{\asprg}\rangle \bigstep \omega_2'
}
\]
...
\]
Observe that the derivation trees ending in $\langle\omega_1,\asprg\rangle \bigstep \omega_1''$ and $\langle\omega_1'',\pwhile{\ivr}{\asprg}\rangle \bigstep \omega_1'$ have exactly one fewer step than that of $\langle\omega_1,\asprg;\pwhile{\ivr}{\asprg}\rangle \bigstep \omega_1'$ (and likewise for the derivations involving $\omega_2, \omega_2'', \omega_2'$).
The inductive hypothesis applies to the derivation of each:
\begin{itemize}
\item From $\langle\omega_i,\asprg\rangle \bigstep \omega_i''$, $i \in \{1,2\}$: If $\omega_1 \approx_{\Gamma,\lowsec} \omega_2$ and $\lsequent{\Gamma}{\asprg}$, then the inductive hypothesis gives us that $\omega_1'' \approx_{\Gamma,\lowsec} \omega_2''$.
We already know that $\lsequent{\Gamma}{\asprg}$, because $\lsequent{\Gamma, \pc : \lowsec}{\asprg}$ is a premise of the (\irref{whileflow}) rule, and $\Gamma(\pc)$ must have been $\lowsec$ because $\lowsec = \ell \lub \Gamma(\pc)$ (where $\lsequent{\Gamma}{\ivr : \ell}$, and $\lowsec$ is the bottom element of the security lattice.
Therefore, $\omega_1'' \approx_{\Gamma,\lowsec} \omega_2''$.
\item From $\langle\omega_i'',\asprg;\pwhile{\ivr}{\asprg}\rangle \bigstep \omega_i'$, $i \in \{1, 2\}$: If $\omega_1'' \approx_{\Gamma,\lowsec} \omega_2''$ and $\lsequent{\Gamma}{\asprg;\pwhile{\ivr}{\asprg}}$, then $\omega_1' \approx_{\Gamma,\lowsec} \omega_2'$.
We just reasoned in the previous bullet that $\omega_1'' \approx_{\Gamma,\lowsec} \omega_2''$, and that $\lsequent{\Gamma}{\asprg}$.
We have from the premise of the current case of the soundness proof that $\lsequent{\Gamma}{\pwhile{\ivr}{\asprg}}$.
So by the (\irref{composeflow}) typing rule, we can conclude that $\lsequent{\Gamma}{\asprg;\pwhile{\ivr}{\asprg}}$.
\textbf{This leaves us with $\omega_1' \approx_{\Gamma,\lowsec} \omega_2'$.}
\end{itemize}
To summarize the case where $\ell' = \lowsec$, we reasoned that executions under both $\omega_1$ and $\omega_2$ would take the same branch of the loop.
If they both take the $\lfalse$ branch, then because $\omega_1 \approx_{\Gamma,\lowsec} \omega_2$ and the final states $\omega_1', \omega_2'$ will be identical to the initial states, noninterference must hold.
If they both take the $\ltrue$ branch, then we reason inductively that as long as noninterference holds for the smaller derivations needed to account for first executing the loop body once, and subsequently executing the remaining $n-1$ iterations of the loop, then it must hold for the entire execution of the loop as well.

\item[Case $\ell' = \hisec$:] In this case, the label of \pc used to type $\asprg$ is \hisec, either because the \pc in which the loop executes is already \hisec, or because $\lsequent{\Gamma}{\ivr : \hisec}$. 
The only way that the \hisec content of \pc can influence the \lowsec content of a final state is if an assignent to a variable $x$ labeled $\Gamma(x) = \lowsec$ occurs within $\asprg$.
The confinement lemma (Lemma~\ref{lemma:confinement}) tells us that for any variable $x$ assigned in $\asprg$, $\hisec \posetleq \Gamma(x)$, and thus $\Gamma(x)$ must not be $\lowsec$. 
So for every $x$ where $\Gamma(x) = \lowsec$, $\omega_1(x) = \omega_1'(x)$ and $\omega_2(x) = \omega_2'(x)$, because $x$ is never assigned in $\asprg$. 
Using the assumption $\omega_1 \approx_{\Gamma,\lowsec} \omega_2$, we can then conclude that $\omega_1' \approx_{\Gamma,\lowsec} \omega_2'$, completing the proof for this case.
\end{description}

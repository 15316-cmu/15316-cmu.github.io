...
\begin{task}[10 points]\rm
  ...
  Label your affirmations so they can be referenced in later tasks.
\end{task}
...
\begin{task}[10 points]\rm
  ...
  can be referenced in later tasks.
  \begin{enumerate}
...
\begin{task}[10 points]\rm
  Suppose we add the following affirmation to the policy:
  \[
    \mi{kmann} \says \ms{member}(\mi{studentA}, \mi{316\_ta})
  \]
  \begin{enumerate}
  \item Can we derive that $\mi{studentA}$ may read file $\mi{316\_roster}$?
    Explain why or why not, referring to specific policy rules from the
    previous tasks.
  \item What affirmation(s) would need to be added to authorize $\mi{studentA}$
    to read $\mi{316\_roster}$?
  \end{enumerate}
\end{task}
